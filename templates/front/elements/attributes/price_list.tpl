...
{* Defined first two options as default in order to avoid null value later
 * This should of course never happen !
*}
{assign var='opt' value=array()}

{foreach $step->options as $option}
    {if $option->id_configurator_step}
        {if count($opt) < 2}
            {append var='opt' value=$option}
        {/if}
    {/if}
{/foreach}
...
{if $step->price_list_display eq 'input'}
    ...
    <div class="row">
        ...
        *}
        ...
        {include file="./price_range_core.tpl" step=$step option=$opt[0] min=$value_x_min max=$value_x_max dimension="1"}
        {if isset($opt[1]) && is_array(reset($pricelist))}
            ...
            {include file="./price_range_core.tpl" step=$step option=$opt[1] min=$value_y_min max=$value_y_max dimension="2"}
        {/if}
    </div>
{else if $step->price_list_display eq 'select'}
    ...
    <div class="row">
        ...
        {include file="./price_select_core.tpl" step=$step option=$opt[0] dimension="1" options=$pricelist}
        {if isset($opt[1])}
            {include file="./price_select_core.tpl" step=$step option=$opt[1] dimension="2" options=current($pricelist)}
        {/if}
    </div>
{else}
    <div class="row"  id="step_option_{$step->id|escape:'htmlall':'UTF-8'}">
        <table class="table">
            ...
            <tr>
                <td>
                    <span class="row-title">{$opt[1]->option.name|escape:'html':'UTF-8'}</span>
                    <span class="col-title">{$opt[0]->option.name|escape:'html':'UTF-8'}</span>
                </td>
            ...
            <tbody>
            {assign var='configStep1' value=$opt[0]}
            {assign var='configStep2' value=$opt[1]}
