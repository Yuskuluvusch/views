...
{foreach $step->options as $option}
	...
	{if !isset($step_have_default) || !$step_have_default}
		<option data-step='{$step->id|escape:'htmlall':'UTF-8'}' value="">{if $step->default_value_select }{l s= $step->default_value_select mod='configurator'} 
                                                                            {else}{l s='Choose a value in the list' mod='configurator'} {/if}</option>
	{/if}
	{foreach $step->options as $option}
		{if $option}
